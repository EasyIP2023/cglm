...
.. c:function:: void glm_mat2x4_vmul(vec4 r, mat2x4 m, vec2 dest)

    multiply vec4 (row vector) with mat2x4 and store in dest row vector

    Parameters:
      | *[in]*  **r**     vec4 (left, row vector)
      | *[in]*  **m**     mat2x4 (right)
      | *[out]* **dest**  destination (result, row vector)

    .. csv-table:: row vec3 (4x1)
        :header: "", "column 1", "column 2", "column 3", "column 4"
    ...
    .. csv-table:: row vec2 (2x1)
        ...
        "row 1", "v0 * m00 + v1 * m01 + v2 * m02 + v3 * m03", "v0 * m10 + v1 * m11 + v2 * m12 + v3 * m13"
