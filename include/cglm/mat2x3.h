...
/*
 ...
 Functions:
   ...
   CGLM_INLINE void glm_mat2x3_vmul(vec3 r, mat2x3 m, vec2 dest);
...
/*!
 * @brief multiply matrix with row vector and store in dest row vector
 *
 * @param[in]  r    vector (left, row vector)
 * @param[in]  m    matrix (right)
 * @param[out] dest result vector
 */
CGLM_INLINE
void
glm_mat2x3_vmul(vec3 r, mat2x3 m, vec2 dest) {
  float v0 = r[0], v1 = r[1], v2 = r[2];

  dest[0] = v0 * m[0][0] + v1 * m[0][1] + v2 * m[0][2];
  dest[1] = v0 * m[1][0] + v1 * m[1][1] + v2 * m[1][2];
}
