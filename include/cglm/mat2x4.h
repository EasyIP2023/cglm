...
/*
 ...
 Functions:
   ...
   CGLM_INLINE void glm_mat2x4_vmul(vec4 r, mat2x4 m, vec2 dest);
...
glm_mat2x4_mul(mat2x4 m1, mat4x2 m2, mat4 dest) {
  float a00 = m1[0][0], a10 = m1[1][0],
        ...
        a03 = m1[0][3], a13 = m1[1][3],

        b00 = m2[0][0], b10 = m2[1][0], b20 = m2[2][0], b30 = m2[3][0],
        b01 = m2[0][1], b11 = m2[1][1], b21 = m2[2][1], b31 = m2[3][1];
...
/*!
 * @brief multiply row vector with matrix and store in dest row vector
 *
 * @param[in]  v    vector (left, row vector)
 * @param[in]  m    matrix (right)
 * @param[out] dest result vector
 */
CGLM_INLINE
void
glm_mat2x4_vmul(vec4 r, mat2x4 m, vec2 dest) {
  float v0 = r[0], v1 = r[1], v2 = r[2], v3 = r[3];

  dest[0] = v0 * m[0][0] + v1 * m[0][1] + v2 * m[0][2] + v3 * m[0][3];
  dest[1] = v0 * m[1][0] + v1 * m[1][1] + v2 * m[1][2] + v3 * m[1][3];
}
